...
const inputWrapperClassName = cn(
  'bg-ink-700 has-[:focus-visible]:outline-accent relative w-full rounded-[4px] shadow-[inset_0_1px_0_rgba(42,58,86,.55)]',
  'has-[:focus-visible]:outline has-[:focus-visible]:outline-2 has-[:focus-visible]:outline-offset-2',
);

const inputClassName = 'w-full bg-transparent outline-none text-text-hi text-[16px] p-[16px] placeholder:text-text-low';
...
export const SmoothInput = ({
  ...
  const springCaretX = useSpring(
    caretX,
    prefersReducedMotion ? { stiffness: 10000, damping: 100, mass: 0.1 } : CARET_SPRING,
  );
    ...
    <div className={cn(inputWrapperClassName, wrapperClassName)}>
      ...
      >
        <input
          ...
          className={cn(inputClassName, 'col-start-1 col-end-2 row-start-1 row-end-2 text-inherit', className)}
          style={style}
          value={inputValue}
          onChange={(e) => {
        ...
        />
        <motion.div
          className="bg-accent pointer-events-none col-start-1 col-end-2 row-start-1 row-end-2 h-[1.1em] w-0.5 self-center"
