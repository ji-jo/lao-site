...
const Footer = () => {
  ...
  React.useEffect(() => {
    ...
    // The line reaches the card around this point. Reading the existing
    // MotionValue avoids a second scroll listener and the old per-scroll path
    // sampling loop (which could miss the contact on fast scrolls).
    if (scrollYProgress.get() >= 0.56) activateSun();
    const unsubscribe = scrollYProgress.on("change", (progress) => {
      if (progress >= 0.56) activateSun();
    });

    // Cover anchor jumps and restored scroll positions by activating as soon
    // as the actual card approaches the viewport.
    const card = footerCardRef.current;
    const observer = card
      ? new IntersectionObserver(
          ([entry]) => {
            if (entry?.isIntersecting) activateSun();
          },
    ...
    >
      <div className="sticky top-0 z-10 mt-16 flex h-[100svh] w-full flex-col items-center justify-start gap-5 text-center md:relative md:mt-24 md:h-auto md:w-fit">
        ...
        <p className="font-body relative z-10 max-w-2xl text-xl text-text">
          Join the LAO waitlist for early access.
        </p>
      ...
      <div className="h-[40vh] w-full shrink-0 md:hidden" aria-hidden="true" />

      <div ref={footerCardRef} className="relative z-10 flex min-h-[420px] w-full max-w-[1200px] flex-col overflow-hidden rounded-[2rem] bg-ink-800 px-8 pb-12 pt-10 font-body text-text-hi shadow-2xl md:min-h-[560px] md:translate-y-[calc(200vh-24px)] lg:px-12">
